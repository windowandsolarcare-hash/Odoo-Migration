...
- **Lead** — PURE OVERSEER (does NOT hand-write feature code): orchestrates all sessions, owns the DESIGN/specs + cross-stream continuity + brand consistency + the domain/DNS seams, QCs every stream, and runs independent fresh-eyes reviews (throwaway reviewer agents) at each launch gate. **★ NOTE (2026-09-02): the LOCAL Lead session (this one) is the current live Lead.** A second "Lead 2" cloud session was appointed interim when DJ's local machine lost power ~2026-08-30; local is back up and Lead 2 is now running an AUDIT (not writing directives). Local Lead holds continuity; treat Lead 2 as a reviewer, not a parallel director. Only one Lead directs at a time.
...
`→ All` = all six roles. Keep file ownership clean (one owner per file) — that is how we avoid the collisions we hit on 2026-08-17.
...
3. **Stripe access:** There is **NO Stripe MCP installed** (verified 2026-09-02 on a fresh session — the loaded MCP servers are render, Gmail, Google Calendar, Google Drive, QuickBooks, Zapier, Calendly, claude-in-chrome; config lives in `C:\Users\dj\.claude.json`, NOT `.claude/mcp.json`). Ignore the old `reference_stripe_mcp` note claiming otherwise. Use the **live key + direct Stripe REST API** (`https://api.stripe.com/v1/...`, `Authorization: Bearer <key>`): key is in Google Drive **Saunders Vault** → doc "Stripe", and local `C:\Users\dj\_stripe_key_val.txt`. Read the key from the file (don't inline it in a shell command — the classifier blocks live `sk_live_` tokens). **Never paste the key in chat or commit it to the repo** (GitHub secret-scanning blocks it — see `project_memory_mirror_secret_scanning`; redact secret SHAPES to placeholders in any memory note).
