...
- **`gh` CLI is NOT installed in cloud shells.** Push via the **GitHub MCP Contents API** (same Contents PUT `gh api` does — satisfies the `main` ruleset). For SMALL files the Contents API is fine; for BIG ones use **`git push` a branch → PR → squash-merge**, which has NO size limit and is proven (445 KB AGENT_MAIL.md, 2026-08-31) — the `main` ruleset protects `main`, not every ref, and git-over-HTTPS is not blocked. The Contents API's ~60–100 KB ceiling is a property of THAT path only, never of cloud sessions ([[feedback_cloud_push_size_limit]]).
- **Cloud outbound network: OPEN on our Default env — TEST IT, do not assume (re-verified live 2026-08-31).** A cloud Lead re-ran the check this session: `wsc-field-assistant.onrender.com/healthz` → 200, `window-solar-care.odoo.com` → 200, `api.render.com` → 401 (reachable, just unauthenticated). **So a cloud session CAN smoke-test the live app, verify by content, and query Odoo.** Two sessions in a row have now repeated a stale limit from this file instead of spending five seconds on `curl` — if you are about to tell DJ you *cannot* do something, test it first ([[feedback_verify_limits_before_declaring]]). The original GitHub-only text, still true for a BRAND-NEW environment whose allowlist has not been opened:
- **(historical default)** GitHub-only until the allowlist is opened. `wscare.pro`, `wsc-field-assistant.onrender.com`, and `window-solar-care.odoo.com` all **403 at the proxy**. So a cloud code-session CANNOT smoke-test the live app or query Odoo — the "verify by content, not status code" step must be done by a LOCAL session or DJ, UNLESS the environment's network allowlist is opened to those domains (claude.ai/code → environment settings → Network → Custom allowlist or Full). **Operator-cloud is fully blocked** until that allowlist is opened (it only works by calling the app's HTTP endpoints).
