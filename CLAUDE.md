...
**★ §6 RIDER — SPRINT ALL CODE A→Z, don't drip-feed (DJ 2026-09-26).** Builders build the WHOLE assigned scope BACK-TO-BACK and hand the finished code over; THEN orchestrators decide DEPLOY pacing ("slow deploy" ≠ "slow build"). The ONLY legitimate builder pause = a HARD CODE DEPENDENCY (stage B needs stage A's tested result to EXIST) — and even then the builder does NOT self-defer: surface "can't start B, need A" UP → orchestrator records it on the board + reissues B the instant A lands. Context-degraded? RESTART fresh + IMMEDIATELY continue — never defer the remaining scope. ★ ORCHESTRATOR WARNING (Lead/Dispatcher): do NOT drip-feed "next piece after each deploy" — that makes the orchestrator the bottleneck (we're AI too; put-off work degrades reliability, builder OR orchestrator). Tracking/re-surfacing is the BACKSTOP, not the plan — the plan is do-it-NOW. Deploy pacing + Dispatcher's money-check stay orchestrator/safety, never a reason to pause the BUILD.
...
10. **WE WORK ON V2 FILES ONLY. Most owner screens were migrated to a "V2" redesign (`static/owner/v2_*.html`), and the V2 file is what DJ's daily launcher actually opens — so the matching legacy non-`v2_` file is DEAD to us unless proven otherwise.** Before editing ANY owner screen: check whether a `v2_<name>.html` twin exists. If it does, edit the **V2** file — editing the legacy twin means DJ won't see your change (burned 2026-07-30: added an "Edit prices" link to `quote.html`, but DJ's launcher opens `v2_quote.html`). Two launchers coexist and this is the tell: the OLD `ql_panel.js` (single-column quick panel) sits on legacy pages; DJ's REGULAR launcher is the v2 **WSCLauncher** (`v2_apps.js`, the 🚀 FAB with **★Favorites / All** tabs) and it links to `/static/owner/v2_*.html`. ★ CORRECTED 2026-09-12: **`field.html` (V1) DOES now have a live V2 twin — `v2_field.html` — and v2_field.html is the LIVE field screen** (every v2 page deep-links to `v2_field.html?open_so=...`: command/maint/myday/etc). Edit the FIELD screen in **`v2_field.html`**, not V1 `field.html`. V1 `field.html` (7,041 lines) is now reached ONLY via the legacy `ql_panel.js:576` → `/owner/field` — a holdout pending a retirement decision, NOT the daily screen. ⚠ The two still DUPLICATE the deep-link/return logic (a wrong-location latch bug had to be fixed in BOTH on 2026-09-12 — see `project_field_deeplink_return_latch`); consolidating/retiring V1 is a pending DJ design call. When in doubt which file a launcher opens, grep `v2_apps.js` for the app's href before assuming. See [[project_two_quote_pages_two_launchers]] and [[project_v2_launcher_duplicated_stale]].

11. **Customer/job-detail field already EXISTS in Odoo → just build it into the app UI. Do NOT hold on DJ.** DJ 2026-09-26: the customer-detail editor is the app's replacement for Odoo's own field UI. If a field is already in Odoo (present via `fields_get` / the field-names table — e.g. gate code `x_studio_x_gate_code`), exposing/editing/deleting it in the app UI is expected work, not a decision — build and ship it. **ONLY hold on DJ when a genuinely NEW Odoo field (a schema change / new custom field) is required** — creating data-model schema is his call. Don't flag or "park" existing-field UI work as a gap; that's a needless hold (this is what happened with the gate-code field). Ties to [[feedback_never_send_dj_to_odoo]] (every Odoo-editable field needs an app pathway) and [[feedback_existing_odoo_field_build_ui_dont_ask]] / [[feedback_escalate_to_dj_sparingly]].
...
- **★ POST-THEN-NUDGE + SELF-REGISTER ROSTER (Lead blessed 2026-09-04, DJ-directed):** after posting a mail entry to specific role(s), IMMEDIATELY `SendMessage` each a one-liner ("posted to AGENT_MAIL → <title>, read+action") so they act now, not on their next tick. Quick ephemeral Q&A goes direct (local↔local two-way); anything to remember/approve/money/customer-facing still gets MAILED. Refs are perishable → every session **self-registers its current ref+role at session start in `3_Documentation/SESSION_ROSTER.md`** (the direct-message address book) AND **re-stamps its FULL row (current ref + role + Last-checked UTC) on every watcher tick** so a restarted session's new ref SELF-HEALS every 30–40 min (not only at session start) + a stale Last-checked flags an offline role; read the roster to find "who is Web right now." Watchers lengthen to **30–40 min** as a backstop (nudges give immediacy + wake sessions only when there's something = fewer tokens). Cloud = one-way (nudge delivers, replies via mail). **★ ANNOUNCE every cross-session nudge/message in your VISIBLE reply** ("nudged Specialists," "messaged Lead") — the SendMessage tool result isn't reliably shown to DJ, but your typed reply is; state it near your end-of-turn sign-off so DJ can confirm the handoff happened. A silent cross-session send is a miss.
...
- **Web** — owns the public marketing website (Odoo website module) at www.windowandsolarcare.com (wscare.pro is the Render-app domain — customer portal + owner ERP + Cheryl's app, behind link/login — NOT the marketing site). Touches no app code.
...
These are not optional and not "for big files." They are why regressions keep happening (e.g. the Jun-8 stale push that dropped 1,377 lines from field.html, the Apr-30 push that dropped 2,277). Run ALL before every push:
...
3. **REGENERATE `3_Documentation/ENDPOINT_MAP.md` whenever you add / change / rename / remove a route.** One command: `python 3_Documentation/gen_endpoint_map.py <repo_root>` (after `git reset --hard origin/main`), then push the refreshed map **with** your change. That map is the fleet's canonical route inventory (LIVE-vs-DEAD shadow, method, full path, file:line, what-it-does, auth) so a session can find/verify any endpoint **without re-reading router code** — it only delivers that value if it's CURRENT. A stale map that looks authoritative is its own trap (it silently drifted 8 days once because this step was skipped, and a whole night of route-adding work went unmapped — 2026-09-12). Audit re-runs it on cadence as a backstop, but the shipping session owns keeping it fresh on the push that changed routes.
...
- **★ ENDPOINT LOOKUP — CHECK `3_Documentation/ENDPOINT_MAP.md` FIRST (do NOT re-read router code to find a route):** it's the canonical inventory of EVERY route — method, full path, LIVE-vs-DEAD shadow, file:line, what-it-does, auth — auto-generated from the code. This is the reference that gets you to a solution fast instead of grepping/reading routers every time; it exists precisely so we stop re-deriving what we already know. Only fall back to grepping when the map is missing a very-recent route (then regenerate it — see pre-push gate 3). This applies to REVIEW/QC and ANSWERING too, not just building. (map-first rule 2026-09-12)
- **Shadowed routes:** before building on any `/owner/*` route, confirm which file actually serves it — the map's LIVE column, or grep the repo for duplicate `@router.post('/<path>')` defs and check `main.py` include order (first `include_router` under a prefix wins). dashboard.py shadows BOTH hemet.py and field.py. (2026-08-19)
