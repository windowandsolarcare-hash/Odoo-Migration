...
- [project_armed_with_dates.md](project_armed_with_dates.md) — "Armed with dates" — /api/openings returns a customer's next open days + booking link, surfaced on the dialer + text-to-DJ so he has availability mid-call. Phase 1 of self-scheduling.
- [project_customer_self_scheduling.md](project_customer_self_scheduling.md) — DJ's PAYOFF feature: customer self-scheduling (job-detail launch → 3-choice tokened portal page → book/pick/free-pick) + 4-day confirm + gated night-before. Spec: CUSTOMER_SCHEDULING_SPEC.md.
- [project_new_job_via_app_endpoints.md](project_new_job_via_app_endpoints.md) — The exact app HTTP endpoints (the ones the phone calls) to create a customer + job the RIGHT way — real job-number sequence, route-aware scheduler, system confirmation. NOT raw Odoo writes.
- [project_intake_property_as_contact.md](project_intake_property_as_contact.md) — My Day task deep links can pass a PROPERTY record as contact= to New Job — backend resolves Property→parent customer (_resolve_contact in new_job.py).
