...
- [project_phase2_workspace_verified.md](project_phase2_workspace_verified.md) — Phase-2 workspace LIVE: #1 authorship (session_actor keys off partner p, switcher retired), #2 links (wsc.goals.links), #3 deps (next_slots after=+_critical_path), #5 card-first (ideas.html Board=default, flat chat demoted to General/history), #6 card→goal promoted-only links. #4 Gantt=Cheryl. Verified-live not rebuilt (Rule 9).