...
**Part 2 — ANSWER don't list (SHIPPED 2026-09-10, tips 71bb7df + 6219cd5):** after ranking, take top ~5 cards and synthesize a 1-2 sentence DIRECT answer with the source (e.g. "Your ThankYou business card — Canva decision, 2026-09-10"), show related cards below as collapsed "sources." Reuse the existing in-app Haiku path (field.py `_agent_loop` / Haiku tier per [[reference_app_ai_prompts]]) — inject the top cards as facts, no tools. Also needs the v2_memory.html Ask-box to render answer-vs-list. memory_store._synthesize_answer (Haiku, top-5 cards→direct 1-2 sentence answer naming the source), returned as `answer` and SKIPPED when a live pointer already answered; v2_memory.html renders it as a 💡 Answer card on top with the ranked cards below labeled 'sources'. Submit-driven (Enter/button) so one Haiku call per question. See [[project_meeting_distill_truncation_and_rekick]] (the reference[] prompt tweak feeds better cards into this).
