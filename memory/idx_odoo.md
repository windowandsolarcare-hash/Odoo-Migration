...
- [project_chase_bankfeed_reconciliation_model.md](project_chase_bankfeed_reconciliation_model.md) — Chase j6 reconciliation model for the Stale-SOs retool: TWO worlds (app account.payment ALL reconciled-to-invoice vs raw bank.statement.line FEED ALL is_reconciled=False/unmatched). Deterministic per-SO branch = SO.invoice_ids→account.move.payment_state (paid/in_payment=close-out-only; no-invoice/not_paid=record-then-close). payment_state = the double-record guard. _execute_payment doesn't reconcile the feed line → dup-in-ledger + 201 unreconciled lines = recon gap. Matcher input = live bank.statement.line (not the CSV).

- [project_stripe_success_route_shadow.md](project_stripe_success_route_shadow.md) — ★ MONEY-PATH root cause: /api/stripe/success route-shadowed (dashboard.py bugged stub serves, payments.py robust one dead) + webhook backstop UNWIRED (no STRIPE_WEBHOOK_SECRET, no app endpoint in Stripe) → app-checkouts strand (Bob/Angela). Fix=un-shadow + webhook-live + anti-double-book vs Odoo provider. journal 6.
