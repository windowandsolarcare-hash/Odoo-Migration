...
- **Price (DJ ruled 2026-09-14, shipped portal.py 4b64ad87 / portal.html 86f47cb9):** care_store resolves `price` (float|None) + `price_note` at read time from DJ-editable `ir.config_parameter wsc.care.prices` {issue:{price,note}} with per-item `price_override`/`price_note_override` winning; portal passes through via `_care_price()` — >0 → "$45.00" (+note), None/0 → "Price TBD — we'll confirm before any work." Never $0/blank. Owner price-editor = Specialists' fast-follow; zero portal change needed.
