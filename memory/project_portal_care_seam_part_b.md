...
- **Care + Before/After** read via `routers.care_store` (**Specialists owns it — import only, never edit; no delete function exists**). Imported LAZILY (`_care_store()`) because `main.py` imports portal.py at boot — a hard import of a missing module = whole app down. Isolation lives in the store (`items_for_partner`/`beforeafter_for_partner` filter by partner_id); portal asks only for `account()['ids']`.
...
- Test data: `care_store.add_care_item(partner_id=26339 "Fred Test", ...)` creates; cleanup = `_get_list`/`_set_list` dropping only your ids (no delete API). Heavy-history customers: Nick Conway contact 22986 (28 Done), Blair Irene Becker contact **23046** (24; property 24169) — 23376 is a different Becker record with 6.
