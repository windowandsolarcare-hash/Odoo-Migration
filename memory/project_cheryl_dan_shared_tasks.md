...
metadata: 
  ...
  modified: 2026-09-10T23:31:01.925Z
---
...
**Two-tab Tasks board + Cheryl New-Project (2026-09-10, DJ-directed):** /cheryl/tasks is now a **Cheryl | Dan** two-tab board (tasks.html, dark-default themed via `cheryl_theme` like documents.html). Cheryl tab = `/api/tasks/list` (her open + unsorted pool, editable). "Give to Dan" no longer vanishes it — same reassign moves it to the **Dan tab** = new `/cheryl/api/tasks/dan_list` (tag 27 + x_owner=3, INCLUDES state 1_done with a struck-through/✓ indicator) so she watches a handed task flip to done when DJ completes it (same project.task). **Cheryl New-Project:** project.project has NO x_owner, so a NEW **"Cheryl" project.tags id=28** (separate from Goal=25 and "Cheryl & Dan"=27 — solo projects ≠ shared tasks) marks a project as hers. `goals.create_goal_core(title,wish,target,cheryl=False)` is the ONE create (goals_create delegates; cheryl=True stamps [25,28]); endpoint POST `/cheryl/api/projects/create` (cheryl/hud.py). The ＋ New-project BUTTON on plan-views.html is Cheryl's-cloud's (they own the fragment). Cheryl HUD "Waiting on you"/stale cards no longer link to the external claude.ai Workbench — now `/cheryl/plan` (feed_live.py `_WORKBENCH_HREF`); an in-app questions[] view is a Cheryl's-cloud gap.

**view_plan role-by-mount-prefix fix** (`goals.py`, DJ-decided "safe fix"): the SAME `view_plan` is served at BOTH `/owner/api/view/plan` (owner_goals.router) AND `/cheryl/api/view/plan` (cheryl_hud.py `add_api_route`). Role signal = **`request.url.path.startswith('/cheryl')`** → `_build_view_plan(..., cheryl_only=True)`. ORIGINALLY (2026-09-09) that returned gids=[] (always empty) to hide DJ's goals; UPDATED 2026-09-10 to return Goal-projects ALSO carrying the **"Cheryl" tag (28)** so HER created projects show while DJ's (Goal-only) stay hidden. days=[] either way (no DJ field-capacity chart on her Plan). This ALSO ended the old sample-data fallback. `/owner` unchanged with DJ's full goals. NO goals deleted/unlinked. This mount-prefix role check is a reusable pattern for any endpoint shared across /owner and /cheryl. See [[feedback_reuse_canonical_endpoint]], [[feedback_never_send_dj_to_odoo]], [[project_cheryl_plan_views_sync]].
