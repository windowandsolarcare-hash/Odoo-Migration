...
- Parses each `| **<Role>** | <ref> | <kind> | <Last-checked> | … |` row (split on `|`, role from `**…**` in cell[0], timestamp `YYYY-MM-DD[ HH:MM]` from cell[3] — time optional, defaults 00:00, so a date-only/malformed old row is still caught). Flags role if `now - Last-checked > _FLEET_STALE_HOURS` (=4).
