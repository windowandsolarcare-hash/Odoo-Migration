...
- [feedback_no_inventing_customer_lines_hud_confirm.md](feedback_no_inventing_customer_lines_hud_confirm.md) — ★ NEVER invent one-off customer message lines (doesn't scale); reschedule/customer-facing changes AUTO-QUEUE a standardized HUD confirm DJ presses to send. Record-only timeline "Rescheduled to X" entries ARE wanted + must be automated (they are).
