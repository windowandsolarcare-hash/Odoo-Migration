...
- [feedback_lead_delegate_dont_do.md](feedback_lead_delegate_dont_do.md) — ★ Lead DELEGATES investigate→diagnose→fix to the owning session immediately (don't go heads-down); stays interface + QC gate; background agent for quick facts. Don't make DJ wait on Lead's keyboard.
...
- [feedback_recordings_chunk_stream_durable.md](feedback_recordings_chunk_stream_durable.md) — ★ (DJ 2026-09-12) EVERY recording feature must CHUNK-STREAM audio to the server WHILE recording (durable per-chunk ir.attachment) so a dead/slept phone loses nothing — never hold the whole clip on-device + upload on Stop. Reuse the meeting pipeline (chunk/finalize + segmented long-audio _transcribe); one shared recorder for meeting/ideas-mic/voice-notes. Unlimited length + phone-death-safe.
- [feedback_durable_foundation_over_shortcut.md](feedback_durable_foundation_over_shortcut.md) — ★ (DJ 2026-09-15) On FOUNDATION/vision work, don't default to the cheap/temp version — SURFACE shortcut-vs-durable tradeoffs (cost, scale ceiling, upgrade path) and let DJ decide BEFORE building. "I want X" on a foundation = build to LAST, not the quickest literal fix. (Sparked by the Memory Pillar sitting on a v1 JSON-blob backend vs a real DB.)
- [feedback_dispatcher_speed_offload_bookkeeping.md](feedback_dispatcher_speed_offload_bookkeeping.md) — ★ Dispatcher: keep turns ≤~1 min; answer DJ first, offload slow GitHub/board bookkeeping to a scribe helper. Consistent overage = route harder.
