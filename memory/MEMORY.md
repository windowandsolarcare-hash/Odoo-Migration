...
- [feedback_verify_collection_and_live_pipe.md](feedback_verify_collection_and_live_pipe.md) — ★ drift/compile/node/AST-add-only do NOT prove an entry hit the RIGHT collection (authz mis-anchor: beacon landed in TECH_GRANTED_OWNER not PUBLIC_EXACT, all static gates passed). AST-verify membership (in-right-set AND not-in-wrong-neighbor) + prove the LIVE end-to-end pipe post-deploy (fire the real request, confirm the real effect + a control). Green deploy ≠ feature works.
...
- [feedback_no_inventing_customer_lines_hud_confirm.md](feedback_no_inventing_customer_lines_hud_confirm.md) — ★ NEVER invent one-off customer lines; changes AUTO-QUEUE a standardized HUD confirm. ★ GOLD STANDARD (all HUD cards): Approve must OPEN the customer's THREAD with the msg PRE-FILLED (open=<pid>&draft=), DJ reviews-in-context/edits/presses SEND — NEVER one-tap-fire. Record-only "Rescheduled to X" timeline entries ARE wanted + automated.
- [feedback_existing_odoo_field_build_ui_dont_ask.md](feedback_existing_odoo_field_build_ui_dont_ask.md) — ★ Customer/job-detail field: if it ALREADY EXISTS in Odoo, just build it into the app UI (don't hold on DJ); only hold when a NEW Odoo field/schema is needed.
...
- [feedback_no_deploy_during_customer_payment.md](feedback_no_deploy_during_customer_payment.md) — ★ Don't deploy/stack deploys while a customer is mid-payment — a ~60-90s Render restart fails the in-flight Pay fetch = looks like a broken link. Hold deploys in active payment windows; batch pushes, don't stack. (Bob Lis incident 2026-09-18.)
- [feedback_wt_spawn_via_powershell.md](feedback_wt_spawn_via_powershell.md) — ★ Spawn fleet tabs (`wt … cmd /k claude -n <Role> /be-<role>`) via the PowerShell tool, NOT Bash — git-bash mangles the slash-args (/k, /be-*) → silent no-tab (wt still exits 0). Verify in ListAgents. (Builder-2 spawn 2026-09-19.)
- [feedback_respawn_session_procedure.md](feedback_respawn_session_procedure.md) — ★ DJ asks for a FRESH copy of a role (reset transcription/boot-only setting) → session PROACTIVELY recites: (1) DJ types `/rename Old <Role>`, (2) session launches the new tab itself via `wt`, (3) DJ deletes old on green. Never assume he remembers.
- [feedback_peer_message_brevity.md](feedback_peer_message_brevity.md) — ★ Direct SendMessage replies = SHORT (a nudge or "copy"); substance goes in AGENT_MAIL (free until read). Ack a → All broadcast with a ✅ in the ledger, NOT a reply back. Long inbound peer msgs bloat context every turn → wasted tokens + faster compaction. Keep the fast-lane for time-sensitive catches only.
- [feedback_no_conclusion_from_single_test_sporadic.md](feedback_no_conclusion_from_single_test_sporadic.md) — ★ (DJ 2026-09-23) SPORADIC failure = NO "fixed"/"root cause" from a single test (pass OR fail); demand repeated tests + a failure RATE; instrument the unobservable surface (phone client telemetry), baseline→fix→measure the drop. Applies to my own language (single obs = hypothesis, not conclusion).
- [feedback_data_location_odoo_vs_postgres.md](feedback_data_location_odoo_vs_postgres.md) — ★ STANDING RULE (DJ 2026-09-20, I8): money/business-record-of-truth (invoices/payments/SOs/customers/properties/accounting/multi-company) → ODOO; app operational/derived/high-write data (memory/convs/ideas/feed/caches) → RENDER POSTGRES. Seam: Postgres REFERENCES Odoo ids, NEVER duplicates money/master (no split-brain, one owner per fact). SOs stay Odoo; SO-derived views → PG summary caches w/ a freshness rule. Every "where does this live?" follows this.
- [feedback_sporadic_bugs_repeat_test.md](feedback_sporadic_bugs_repeat_test.md) — ★ Sporadic bug: NEVER conclude from ONE test (pass or fail). Repeat cycles → failure RATES + client telemetry on DJ's real phone.
...
- [feedback_lazy_on_demand_not_prefetch.md](feedback_lazy_on_demand_not_prefetch.md) — ★ Never speculatively prefetch; load on open/expand only; cache-first (instant last-known + bg refresh) only where the on-tap wait is unacceptable. Audit existing code before rebuilding.
- [feedback_no_idle_with_unblocked_work.md](feedback_no_idle_with_unblocked_work.md) — ★ Never idle while UNBLOCKED work is undone; idle only when blocked on DJ. Low-priority = done later, never dropped.
- [feedback_builders_dont_sequence_or_defer.md](feedback_builders_dont_sequence_or_defer.md) — ★ ROOT CAUSE: builders NEVER decide sequencing/deferral/pacing/benching (they have no way to re-surface deferred work = dead code). Pacing = Lead/Dispatcher/DJ call. Sweep is only a backstop, not the fix.
- [feedback_restart_sessions_at_fleet_idle.md](feedback_restart_sessions_at_fleet_idle.md) — ★ Restart long sessions during a FLEET-IDLE window, not mid-sprint (swapping mid-active-work risks a handoff drop); if unavoidable, cover the seam + commit handoff first.
