...
## ★ ACTIVATED 2026-09-19 (fleet-only, observe-only) — wiring + a gotcha that LOOKS like a bug
- WIRED into `Migration to Odoo/.claude/settings.json` (project-level = FLEET-ONLY scope; NOT user-level ~/.claude): merged PostToolUse (matcher "Bash") → mem_error_hook.py + Stop → mem_error_capture.py, commands `python3 "$CLAUDE_PROJECT_DIR/.claude/hooks/<script>.py"`. Preserved the existing env/permissions/SessionStart(git-sync). Scripts live at `Migration to Odoo/.claude/hooks/` (local, not-git; repo mirror at Odoo-Migration/3_Documentation/roles/hooks/). OBSERVE-ONLY: MEM_HOOK_INJECT unset → recall-read + observe/resolve run, fix-INJECTION stays OFF (separate later DJ nod). Takes effect for NEW fleet sessions (a session loads settings at start).
- python3 resolves on this env (3.14.3); hook-exec is POSIX sh.
- ★★ NOT-A-BUG GOTCHA: `/hook/recall` returns a record ONLY if `status=='solved' AND fix` (memory_store hook_recall ~:847) — it injects KNOWN FIXES, not open observations. So observe-then-recall of the SAME sig returns `record:null` — that is CORRECT, not a broken round-trip. The real pipeline is observe (status 'open', occurrences++) → **resolve** (Stop hook → /hook/resolve → Haiku distills root_cause+fix → status 'solved') → THEN recall returns it. Proved live end-to-end 2026-09-19 (observe occurrences 1→4; resolve → solved w/ Haiku fix; recall returned the solved record, company_id='fleet'). Don't re-diagnose the observe→recall→null as a store bug.
- DAL confirmed persistent (ir.config_parameter, no in-memory cache); mem_put stamps company_id='fleet'; re-observe increments occurrences (write persists). Fail-open verified (garbage stdin → exit 0).
