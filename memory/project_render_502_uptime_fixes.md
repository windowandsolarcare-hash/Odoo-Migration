...
**How to apply:** when 502s/uptime come up, check [[project_render_deploy_failed_check]] first (failed deploy serving stale), then memory via `get_metrics`. When adding AI code, reuse the singleton. Related: [[reference_render_tools_architecture]], [[feedback_render_env_var_patch_not_put]].
