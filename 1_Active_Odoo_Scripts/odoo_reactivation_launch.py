# ==============================================================================
# ODOO REACTIVATION ENGINE - OPPORTUNITY CREATION (v3.1 - FINAL)
# ==============================================================================
#
# PURPOSE:
# This script identifies dormant customers from historical sales orders and creates
# reactivation opportunities in Odoo. It automatically calculates updated pricing
# based on historical services, generates personalized SMS messages, and triggers
# a Zapier webhook to send the message via Workiz.
#
# WORKFLOW:
# 1. Reads source sales order (dormant customer)
# 2. Extracts customer contact and service history
# 3. Detects all services previously performed
# 4. Applies 5% annual price increase (compounded) to non-solar services
# 5. Generates Calendly booking link with prefilled customer data
# 6. Creates SMS message body with updated pricing
# 7. Creates new CRM Opportunity with expected revenue
# 8. Updates contact's last reactivation date (cooldown tracking)
# 9. Triggers Zapier webhook to send SMS via Workiz
#
# KEY FEATURES:
# - Intelligent service detection from order history
# - Price engine with 5% annual compound increase
# - Solar service pricing (no inflation)
# - City-based Calendly event routing
# - URL prefilling (name + address)
# - Product exclusions (tip, discount, legacy, quote)
# - Minimum price floor ($85)
# - Workiz UUID reference for graveyard job linking
#
# INTEGRATION POINTS:
# - Odoo CRM (creates crm.lead opportunity)
# - Zapier (webhook trigger for SMS)
# - Workiz (SMS delivery via graveyard job)
# - Calendly (booking link with prefilled data)
#
# DEPLOYMENT:
...
for source_order in records:
    # DEBUG: Post to chatter to confirm LAUNCH script is running
    source_order.message_post(body="[DEBUG] LAUNCH script started - creating opportunity and triggering webhook...")
