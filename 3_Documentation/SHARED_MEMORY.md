...
Customer lookup: `_find_customer` (sms.py) now matches the primary Phone OR the "Second Phone" field (`x_studio_x_studio_second_phone`, last-7 ilike). So a customer calling/texting from a 2nd number is recognized — add their alt number to Second Phone (pure digits). Recordings listenable via `/owner/voice/rec?sid=`; diag list at `/owner/voice/recordings?token=CRON_SECRET`.
Voice Intelligence call TRANSCRIPTS now WORK (fixed 2026-08-11): the `Channel` payload must nest source_sid under `media_properties` ({"media_properties":{"source_sid":"RE.."}}) — before it was sent flat so Twilio made nothing. Transcript lands async on the caller's Odoo chatter + inbox thread as "📄 Incoming call transcript:".
...
- **NOW — Approve REUSES the re-engagement job:** new field `res.partner.x_reengagement_workiz_uuid` is stamped at SMS-send time (reactivation.py `/api/followup/launch`). On Approve (booking_requests.py): if the contact has a live re-eng job (uuid set, Workiz Status not Done/Canceled) → it **schedules THAT job in place** (job/update: date/JobType/type_of_service_2=Maintenance/SubStatus="Send Confirmation - Text", keeps the text), **links the booking SO to it + confirms it** (lands on schedule), clears the pointer. Else → old clone path (SA 1338). Forward-only (only re-engagements sent after 2026-06-29 get the stamp).
- **Approve is now ONE TAP** → copies line items to clipboard history (price then name) + **auto-opens the Workiz /items tab** (`app.workiz.com/root/job/<uuid>/items`). No second "Open in Workiz" tap. Done screen has a "Reopen Workiz" fallback link.
- **Approve marks the My Day booking reminder DONE** (finds the `project.task` whose description has `booking_requests?focus=<so_id>)`, sets `state='1_done'`). Reminder also has a "📥 Review & Approve Booking →" button that deep-links to the request (added 2026-06-28).
- **WORKIZ "Canceled" is a top-level STATUS, not a SubStatus.** To cancel a job: `job/update {Status:'Canceled'}` (NO SubStatus — `SubStatus:'Canceled'` → 400). Top-level Statuses = Submitted/Done/Canceled/In Progress.
...
- **2026-07-18 v2_field shipped**: /static/owner/v2_field.html = v2 Field Day (schedule + full job panel + timer + clock-in/break/end-day + offline photo queue). Shares wsc_timer / wsc_break / wsc_crew_today / wsc_pending_clockin-out / wsc_pq with the LIVE field app, so state crosses over. v2_job also got the wsc_pq queue-first photo upload. Live app untouched.
- **2026-07-19 Calendly RETIRED 100%** (DJ order): never put calendly.com in any SMS. Booking links = wscare.pro/c/<token> (make_token in routers/booking.py) or wscare.pro/book fallback. Patched: Odoo SA 562+563 (emit /book; Render regex upgrades to tokenized), reactivation.py, hemet.py AND dashboard.py (dashboard SHADOWS /api/hemet/* — always patch dashboard.py for hemet endpoints). Calendly inbox capture (SA 1340) left passive.

## V2 cutover (2026-07-20)
- /owner/ now redirects to the v2 app (v2_home.html). DJ approved production cutover.
- V1 fully intact as fallback: /owner/v1 = old hub; all old /owner/* page routes still serve v1 pages. No v1 files replaced/deleted.
- Rollback = remove the two routes at the top of main.py (registered before include_router).
