...
- **UI labels = the existing app's vocabulary, NOT Odoo field names.** Before labeling any data point in a new screen, check what the *old* UI called it and reuse that exact term. Confirmed examples: **"Total"** (never "Line total"), **"Services / Line Items"** (never "lines"), **"Status"**, **"Tech"**, **"Scheduled"**, **"Gate"**, **"Pricing"**, **"Frequency"**, **"Type of Service"**. When unsure, grep the old `static/owner/*.html` for the label rather than inventing one.
